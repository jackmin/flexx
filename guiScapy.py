...
nsamples = 16
p_list = []
t = AsyncSniffer(iface="en0", prn=lambda x: p_list.append(x))
...
class MessageBox(flx.Label):
    ...
    def add_message(self, name, msg):
        ...
        self.set_html(line + self.html + '<br />')
...
class Relay(flx.Component):
    ...
    def print_packet(self):
        ...
        for i in range(self.prev_idx, self.curr_idx):
            self.txt_packet += p_list[i].summary() + '\n'
        self.prev_idx = self.curr_idx
        return self.txt_packet
...
class MonitorView(flx.VBox):
    ...
    def init(self):
        ...
        self.status = MessageBox(flex=1)
        #self.status = flx.Label(text='...', wrap=1)
    ...
    def update_info(self, info):
        ...
        #                     'and blablablabla.<br /> ')
        #self.status.set_html(info.packets)
        self.status.add_message(info.packets)
...
if __name__ == '__main__':
    ...
    flx.start()
#a = list(psutil.net_if_addrs().keys())
#p_list = []
#t = AsyncSniffer(iface="en0", prn=lambda x: p_list.append(x))
#t.start()
